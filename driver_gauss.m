% This script shows how to run urOMT algorithm and Eulerian & Lagrangian post-processing on a gaussian dataset (which consists of 5 successive images)
...
z = 1:cfg.true_size(3);
figure,
tmp = reshape(mean(Eul.Es,2),cfg.true_size);
...
% Eul averaged r
figure,
tmp = reshape(mean(Eul.Er,2),cfg.true_size);
tmp(tmp==0) = NaN;
hs=slice(y,x,z,tmp,y,x,z); 
set(hs,'EdgeColor','none','FaceColor','interp','FaceAlpha',0.04);
...
cb = colorbar;
cb.Ticks = linspace(-0.02, 0.02, 5);
cb.TickLabels = num2cell(-0.02:0.01:0.02);
cb.FontSize = 20;
...
cb = colorbar;
cb.Ticks = linspace(0, 1, 6);
cb.TickLabels = num2cell(0:0.3:1.5);
cb.FontSize = 20;
text(1,-3,15,'speed (a.u.)','Rotation',90,'FontSize',25);
...
cb = colorbar;
cb.Ticks = linspace(0, 1, 6);
cb.TickLabels = num2cell(0:200:1000);
cb.FontSize = 20;
