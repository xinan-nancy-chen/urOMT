function [drNdrTdrNdr] = get_drNdrTdrNdr(M,nt,dt,par,x)
%% Created by Xinan Chen in Nov 2021
%% This function returns (d(rho(:,end))/d(r))^T * d(rho(:,end))/d(r) * a vector x
...
for i = 1:nt
    ...
    if  i>1
        for j = 1:i-1 %parfor
            ...
            if pcgflag2 ~= 0
                warning('MATLAB:pcgExitFlag','Warning: get_drNdrTdrNdr.m >>> while finding drho(:,n)/du_%d, pcg exit pcgflag2 = %d',j,pcgflag2)
            end
        end
    end
    ...
    if pcgflag3 ~= 0
        warning('MATLAB:pcgExitFlag','Warning: get_drNdrTdrNdr.m >>> while finding drho(:,%d)/du, pcg exit pcgflag3 = %d',i,pcgflag3)
    end
end
...
for i = nt:-1:1
    ...
    if pcgflag1 ~= 0
        warning('MATLAB:pcgExitFlag','Warning: get_drNdrTdrNdr.m >>> while finding drho_%dT/du, pcg exit flag1 = %d',i,pcgflag1)
    end
